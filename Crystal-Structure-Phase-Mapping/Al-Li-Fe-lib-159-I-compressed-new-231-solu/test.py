...
def plot_recon(xrd, JS_dis_batch, L2_dis_batch, degree_of_freedom, comp_loss_batch, composition, comp_prime, noise, gibbs_loss_batch, alloy_loss_batch, basis_weights, avg_recon_loss, recon_loss, decomp, xrd_prime, mu, mu_shift, logvar, intensity):
    ...
    if ("binary" in sys.argv):
        examples = binary_examples + examples
    ...
    for i in examples:
        ...
        cnt = 0
        for j in idx:
            if (basis_weights[i][j] >= 1e-3 and j in top_bases):
        ...
        if ("savefig" in sys.argv):
            plt.savefig("figs/sample_%d.png"%(i + 1))
            plt.close('all')
...
def plot_ternary(xrd, degree_of_freedom, basis_weights, comp_loss_batch, composition, comp_prime, noise, gibbs_loss_batch, avg_recon_loss, recon_loss, decomp, xrd_prime, mu, mu_shift, logvar, intensity):
    ...
    f, axes = plt.subplots( n_row, n_col, figsize = (3.5*n_col, 3.5*n_row))
    ...
    for j in range(n_top_bases):
        ...
        axes[u][v].scatter(X, Y, s = (6.0)**2 , c = "#eeefff")

        rgb = np.tile([colors[j]], [len(X), 1])

        axes[u][v].scatter(X, Y, s = basis_weights[:, b]*(6.0)**2 , c = rgb) #
        axes[u][v].legend(["basis-%d, activation = %.3f"%(b, mean_basis_weights[b])], loc = "upper right")
    ...
    X = coords[:, 0]
    Y = coords[:, 1]
    axes.scatter(X, Y, s = (15.0)**2 , c = "gray", alpha = 1.0)
    ...
    for i in range(N):
        for j in range(N):
            if (edges[i][j] == 1):
                if (np.sum(np.abs(sample_colors[i] - sample_colors[j])) < 1e-6):
                    #print(i, j)
                    axes.plot([X[i], X[j]],  [Y[i], Y[j]], c = "black", alpha= 0.5, zorder = -1)

                #axes.plot([X[i], X[j]],  [Y[i], Y[j]], c = "black", linestyle='dashed')

    axes.scatter(X, Y, s = (10.0)**2 , c = sample_colors , zorder = 1) #
    """
...
def check_connectivity(xrd, degree_of_freedom, basis_weights, comp_loss_batch, composition, comp_prime, noise, gibbs_loss_batch, avg_recon_loss, recon_loss, decomp, xrd_prime, mu, mu_shift, logvar, intensity):
    ...
    phase_dict = {}
    for i in range(1, (1<<n_top_bases)):
        tmp = []
        for j in range(n_top_bases):
            if ((i>>j)&1 == 1):
                tmp.append(top_bases[j])

        phase_dict[tuple(tmp)] = 1
        ...
        if (np.sum(v) > 0):
            ...
            if (is_connected(edges, v)):
                ...
                if ("visual" in sys.argv):
                    ...
                    X = coords[V, 0]
                    Y = coords[V, 1]
                    axes.scatter(coords[:, 0], coords[:, 1], s = (10.0)**2 , c = "#eeefff", alpha = 0.5)
...
def generate_solution(xrd, degree_of_freedom, basis_weights, comp_loss_batch, composition, comp_prime, noise, gibbs_loss_batch, avg_recon_loss, recon_loss, decomp, xrd_prime, mu, mu_shift, logvar, intensity, intensity_shift):
    ...
    N = basis_weights.shape[0]
    M = bases_name.shape[0]
    for i in range(N):
        for j in range(i+1, N):
            if (edges[i][j] == 1):
                n_fi = np.sum(np.greater(composition[i], 1e-6).astype("int"))
                n_fj = np.sum(np.greater(composition[j], 1e-6).astype("int"))
                phase_seti = np.greater(basis_weights[i], 1e-6).astype("int")
                phase_setj = np.greater(basis_weights[j], 1e-6).astype("int")
                is_boundary = 1
                if (n_fi == n_fj and np.sum(np.abs(phase_seti - phase_setj)) == 0):
                    is_boundary = 0

                is_shifting = 0
                for k in top_bases:
                    if (phase_seti[k] == 1 and phase_setj[k] == 1 and np.abs(mu_shift[i][k] - mu_shift[j][k]) > 1e-3):
                        is_shifting = 1

                if (is_shifting):
                    wa = 0
                    if (is_boundary and np.sum(phase_seti) == n_fi and np.sum(phase_setj) == n_fj):
                        wa = 1
                    if (is_boundary == 0 and (np.sum(phase_seti) == n_fi or np.sum(phase_setj) == n_fj)):
                        wa = 1
                    if (wa == 1):
                        print(i, n_fi, j, n_fj)
                        print(basis_weights[i][top_bases])
                        print(basis_weights[j][top_bases])
                        print(mu_shift[i][top_bases])
                        print(mu_shift[j][top_bases])

    f = open("solu.txt", "w")
    f.write("K=%d\n" % n_top_bases + "Params=[R,C],[Q,B,C,S],[IS]\n")
