...
def compute_weights_acc(basis_weights, weights_sol, ans_set, V):
    th = 0.02
    accs = []
    acc2s = []
    for i in range(len(basis_weights)):
        if (V[i] == 1):
            ...
            for j in range(len(ans_set)):
                gt[ans_set[j]]=weights_sol[i][j]

            tmp = np.argsort(pred)

            act1 = np.greater(pred, th).astype("int")
...
def main(V, only_idx):
    ...
    learning_rate = tf.train.exponential_decay(FLAGS.learning_rate * fac, global_step, 60000, FLAGS.lr_decay_ratio, staircase=True)
        ...
        for i in range(train_idx.shape[0]):
            ...
            if current_step%FLAGS.check_freq==0: #summarize the current training status and print them out
                ...
                for (k, basis_w) in enumerate(basis_weights):
                    ...
                    #for w in basis_w[:9]:
                    #   print("%3.2f"%w, end = " ")
                    print("| %.3f| %3d| recon = %7.3f| gibbs = %.3f| max_shift = %.6f| alloy = %6.3f| noise = %.3f| r = %.1f|"%(tot, indices[k], recon_loss_batch[k], gibbs_loss_batch[k], max_shift[k], alloy_loss_batch[k], np.mean(noise), gibbs_penalty_ratio[k]), end = " ")
            ...
            if current_step % FLAGS.valid_freq==0: #exam the model on validation set
                ...
                avg_recon_loss = np.mean(st_avg_recon_loss * V)
                ...
                ans_set = [22, 126, 72, 135, 137, 154]
                weights_L1_loss = np.sum(np.sum(np.abs(basis_weights[:,ans_set] - weights_sol), axis = 1) * V)/np.sum(V)
                weights_acc, weights_acc2 = compute_weights_acc(basis_weights, weights_sol, ans_set, V)
                ...
                if (current_step // FLAGS.valid_freq == 100):
                    sess.close()
                    return weights_acc, weights_acc2
