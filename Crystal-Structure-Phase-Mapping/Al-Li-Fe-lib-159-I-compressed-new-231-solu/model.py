...
def decoder_net(input_x, weights_regularizer, epoch, shift_indicator, is_training):
    ...
    #logvar_init = tf.Variable(logvar_init, trainable = False, dtype="float32") #n_bases, n_spike

    #ratio = (epoch / 200.0)*0.99 + 0.01
    ...
    with tf.variable_scope('decoder'):
        ...
        r1 = 1.0 #(tf.minimum(epoch, 10.0) /10.0)
        ...
        #logvar_shift = tf.clip_by_value(logvar_shift, -FLAGS.logvar_range, FLAGS.logvar_range)

        logvar = logvar_shift + tf.Variable(np.zeros(FLAGS.n_bases + FLAGS.n_new_bases) + FLAGS.init_logvar, trainable = False, dtype = tf.float32, name = "logvar")
        
        logvar = tf.reshape(logvar, [-1, FLAGS.n_bases + FLAGS.n_new_bases, 1])
        logvar = tf.tile(logvar, [1, 1, FLAGS.n_spike])
        ...
        intensity_shift = tf.exp(tf.tanh(tf.reshape(x, [-1, FLAGS.n_bases + FLAGS.n_new_bases, FLAGS.n_spike])) * lim)  
        #intensity_shift = tf.exp(tf.clip_by_value(tf.Variable(np.zeros((FLAGS.n_bases + FLAGS.n_new_bases, FLAGS.n_spike)), trainable = True, dtype = tf.float32, name = "I_shift"), -lim, lim)) 
...
def L1_dis(x, y):
    return norm1(x - y) * FLAGS.L1_weight

def L2_dis(x, y):
    return norm2(x - y) * 100 * FLAGS.L2_weight

def JS_dis(input_xrd, xrd_prime):
    return  (0.5 * KL_dis(input_xrd, xrd_prime) + 0.5 * KL_dis(xrd_prime, input_xrd)) * FLAGS.KL_weight
...
class MODEL:
    ...
    def __init__(self, is_training):
        ...
        self.JS_dis_batch = JS_dis(self.normalized_input_xrd, self.xrd_prime)
        ...
        self.alloy_loss_batch = avg_diff * (tf.log(self.degree_of_freedom) - tf.log(self.degree_of_freedom - 1 + 0.1))
        ...
        self.gibbs_loss = tf.reduce_mean((self.gibbs_loss_batch + self.alloy_loss_batch * FLAGS.alloy_loss_decay) * penalty_ratio) # / (FLAGS.eps + tf.reduce_sum(penalty_ratio))
        ...
        self.smooth_weights_loss_batch = tf.sqrt(tf.reduce_sum(tf.maximum(tf.square(self.prev_weights - self.next_weights) - FLAGS.smooth_weights_th, 0.0), axis = 1) + FLAGS.eps )
        
        if (is_training):
            tf.summary.scalar('train/smooth_weights_loss * %.6f'%FLAGS.smoothness_decay, tf.reduce_mean(self.smooth_weights_loss_batch))

        self.smooth_weights_loss_batch /= self.distance #* tf.reduce_min(self.distance)
