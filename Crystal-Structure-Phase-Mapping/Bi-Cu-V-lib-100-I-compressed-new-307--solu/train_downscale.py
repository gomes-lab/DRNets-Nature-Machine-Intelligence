...
def compute_weights_acc(basis_weights, weights_sol, V):
    th = 0.02
    accs = []
    acc2s = []
    for i in range(len(basis_weights)):
        if (V[i] == 1):
            pred = basis_weights[i]
            gt = weights_sol[i]

            tmp = np.argsort(pred)[::-1]

            act1 = np.greater(pred, max(th, pred[tmp[3]])).astype("int")
            act2 = np.greater(gt, th).astype("int")

            if (np.sum(act1) > 3):
                print("WA act1")

            acc = np.sum(np.minimum(act1, act2))/np.maximum(np.sum(np.maximum(act1, act2)), 1e-9)
            acc2 = int(acc == 1)
            accs.append(acc)
            acc2s.append(acc2)
    return np.mean(accs), np.mean(acc2s)
...
def main(V, only_idx):
    ...
    tmp = get_data.get_weights_sol()
    weights_sol = np.zeros((tmp.shape[0], tmp.shape[1] + 1))
    weights_sol[:, :tmp.shape[1]] = tmp
    ...
    for one_epoch in range(FLAGS.max_epoch):
        ...
        for i in range(train_idx.shape[0]):
            ...
            indices = [only_idx, only_idx]
            #for point in batches[train_idx[i]]:
            #    if (V[point] == 1):
            ...
            Round = max(0, (current_step - base_step - pretrain_step) / 100.0)
            ...
            if current_step%FLAGS.check_freq==0: #summarize the current training status and print them out
                ...
                ans_set = [[1,38], [12], [24, 28], [35], [46], [47], [54], [58], [62], [63]]
                #[[1,38], [12], [24, 28], [35], [44], [47], [54], [58], [62], [80]]
                #[[80], [67], [47], [55], [46], [60], [40], [35], [5], [76], [1, 38], [63], [62], [12], [22]]
                for (k, basis_w) in enumerate(basis_weights):
                    ...
                    if (indices[k] == 38):
                        idx_str = color.RED + idx_str + color.END
            ...
            if current_step % FLAGS.valid_freq==0: #exam the model on validation set
                ...
                weights_L1_loss = np.sum(np.sum(np.abs(basis_weights- weights_sol), axis = 1) * V)/np.sum(V)
                weights_acc, weights_acc2 = compute_weights_acc(basis_weights, weights_sol, V)

                print("weights_L1_loss", weights_L1_loss)
                print("weights_acc", weights_acc)
                print("weights_acc2", weights_acc2)

                #idx2 = stats("weights_L1_loss", )
                stats("recon_loss", recon_loss)

                if (weights_acc2 > best_acc2):
                    best_acc1 = weights_acc
                    best_acc2 = weights_acc2

                if (current_step == 3000):
                    sess.close()
                    return best_acc1, best_acc2, basis_weights, weights_sol, V
                #stats("weights_L1_loss", weights_L1_loss)

                summary_writer.add_summary(MakeSummary('valid/max_recon_loss', np.max(recon_loss * V)), current_step)
                summary_writer.add_summary(MakeSummary('valid/avg_recon_loss', avg_recon_loss), current_step)
                summary_writer.add_summary(MakeSummary('valid/weights_L1_loss', weights_L1_loss), current_step)
                summary_writer.add_summary(MakeSummary('valid/weights_acc', weights_acc), current_step)
                summary_writer.add_summary(MakeSummary('valid/weights_acc2', weights_acc2), current_step)
...
if __name__=='__main__':
    N_tot = FLAGS.testing_size

    accs = []
    acc2s = []
    n_rounds = N_tot
    for k in range(n_rounds):
        V = np.zeros(FLAGS.testing_size)
        V[k] = 1

        tf.reset_default_graph()
        acc, acc2, basis_weights, weights_sol, V = main(V, k)
        np.save("basis_weights", basis_weights)
        np.save("weights_sol", weights_sol)
        np.save("V", V)

        accs.append(acc)
        acc2s.append(acc2)
        print("round-%d: acc="%(k), acc, acc2)
        print(np.mean(accs), np.mean(acc2s))

    print("Final acc", np.mean(accs), np.mean(acc2s))
