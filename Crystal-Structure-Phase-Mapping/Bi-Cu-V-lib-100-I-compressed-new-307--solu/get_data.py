...
refined_sample_indicator2 = np.load(FLAGS.refined_sample_indicator_dir[:-4] + "2.npy")
refined_shift_indicator2 = np.load(FLAGS.refined_shift_indicator_dir[:-4] + "2.npy")
...
def get_indicator(my_order, flag = 0, offset = None):
    ...
    # we remove the lattice parameters and G6
    output = []
    known_bases = [80, 67, 47, 55, 46, 60, 40, 35, 5, 76, 1, 63, 62, 12, 22]
    ...
    for i in my_order:
        ...
        #if (i < FLAGS.testing_size):
        #    x *= refined_sample_indicator2[i]
        x[72] = 0 #SnO2
        #for j in range(FLAGS.n_bases):
        #    if (not j in known_bases):
        #        x[j] = 0 
...
def get_refined_sample_indicator(my_order, version = 1, offset = None):
    ...
    for i in my_order:
        if (version == 1):
            ...
            x = refined_sample_indicator2[i][:FLAGS.n_bases + FLAGS.n_new_bases]
        #x = (x - _mean)/_std
        output.append(x)
