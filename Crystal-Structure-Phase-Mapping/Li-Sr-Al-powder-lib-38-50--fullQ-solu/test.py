...
def plot_recon(xrd, JS_dis_batch, L2_dis_batch, degree_of_freedom, comp_loss_batch, composition, comp_prime, noise, gibbs_loss_batch, alloy_loss_batch, basis_weights, avg_recon_loss, recon_loss, decomp, xrd_prime, mu, mu_shift, logvar, intensity):
    ...
    stick_bases = np.load(FLAGS.stick_bases_dir, allow_pickle=True)
    gt_weights_lst = np.load(FLAGS.gt_weights_dir)
    gt_phases = [14, 36, 7]

    gt_weights = np.zeros_like(basis_weights)
    for i in range(gt_weights_lst.shape[0]):
        for (j, idx) in enumerate(gt_phases):
            gt_weights[i][idx] = gt_weights_lst[i][j]

    print("fraction L1 loss", np.mean(np.sum(np.abs(gt_weights - basis_weights), axis = 1)))
    ...
    frac_acc = []
    frac_diff = []
    for i in range(gt_weights.shape[0]):
        for j in gt_phases:
            frac_diff.append(np.abs(gt_weights[i][j] - basis_weights[i][j]))
            frac_acc.append(int(frac_bin(gt_weights[i][j]) == frac_bin(basis_weights[i][j])))
            if (frac_acc[-1] == 0):
                print(i, j, gt_weights[i][j], basis_weights[i][j])

    print("frac_acc:", np.mean(frac_acc))
    stats("frac_diff", frac_diff, True)

    xrd_gt = (xrd[:, Q_idx].T / np.max(xrd[:, Q_idx], axis = 1)).T
    print("recon L1 loss:", np.mean( np.sum(np.abs(xrd_prime - xrd_gt), axis = 1) / np.sum(np.abs(xrd_gt), axis = 1) ) )
    print("recon L2 loss:", np.mean( np.sum(np.square(xrd_prime - xrd_gt), axis = 1) / np.sum(np.square(xrd_gt), axis = 1) ) )
    visual = True
...
def generate_solution(xrd, degree_of_freedom, basis_weights, comp_loss_batch, composition, comp_prime, noise, gibbs_loss_batch, avg_recon_loss, recon_loss, decomp, xrd_prime, mu, mu_shift, logvar, intensity, intensity_shift):
    ...
    fwrite(Y, f)
    f.write("sample_no=")
    fwrite(np.arange(len(Y)), f)
    f.write("plate_id=")
    fwrite(np.zeros(len(Y)), f)
    f.write("pattern_fn=")
    fwrite(np.zeros(len(Y)), f)
