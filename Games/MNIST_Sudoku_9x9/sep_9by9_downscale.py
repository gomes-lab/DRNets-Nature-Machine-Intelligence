...
def gen_components(nums, batch_size, use_cuda):
    ...
    return alldiff_constraints, gen_labels1, gen_labels2

def run(args, indices, sudoku, sudoku5678, ori_label, ori_label5678):
    ...
    #n_data = 1000
    batch_size = min(args.batch_size, n_data)
    ...
    for _epoch_ in range(N_epoch):
        ...
        for idx in indices:
            ...
            if (len(s_mix_lst) == batch_size or idx == indices[-1]):
                ...
                for ii in range(epochs):
                    ...
                    eqn = np.equal(labels12, l12).astype("int").reshape(bs, nums**2, 2)

                    label_acc = np.mean((np.sum(eqn, axis = 2) == 2).astype("float32"))
                    sudoku_acc = np.mean((np.sum(eqn, axis = (1,2)) == 2 * (nums ** 2)).astype("float32"))

                    # compute mixture
                    alldiff_constraints, gen_labels1, gen_labels2 = gen_components(nums, bs, use_cuda)
                    ...
                    gen_img1 = gen_img1.view((nums ** 2) * bs, nums, 1, 32, 32)
                    gen_img2 = gen_img2.view((nums ** 2) * bs, nums, 1, 32, 32)
                    ...
                    gen_mix = gen_mix.view(1, 32, 32, bs * nums * nums, 2, nums)
        ...
        if (_epoch_ % check_freq == 0):
            ...
            for i in range(len(batches)):
                ...
                l12 = np.concatenate([l1.reshape(-1, 1), l2.reshape(-1, 1)], axis = 1) # bs * 16, 2
                eqn = np.equal(labels12, l12).astype("int").reshape(bs, nums**2, 2)
                label_acc = (np.sum(eqn, axis = 2) == 2).astype("float32")
                ...
                for j in range(len(sudoku_acc)):
                    all_sudoku_acc.append(sudoku_acc[j])
                    all_label_acc.append(label_acc[j])

            all_sudoku_acc = np.mean(all_sudoku_acc)
            all_label_acc = np.mean(all_label_acc)

            print("unsolved", unsolved[:10])

            print("#puzzle = %d, sudoku_acc = %f, label_acc = %f, recon_loss = %f (best_acc = %f)"%\
            (len(indices), all_sudoku_acc, all_label_acc, all_recon_loss/cnt, best_acc))
    ...
    return unsolved, all_sudoku_acc, all_label_acc
...
parser.add_argument("--scale_recon", default=0.003, type=float)
parser.add_argument("--mode", default="train", type=str)
parser.add_argument("--model_idx", default="down_scale", type=str)
parser.add_argument("--restart", default=False, type=bool)



steps = [1, 5, 10, 20, 50, 100, 200, 500, 1000, 10000]
bss = [1, 1, 1, 2, 5, 10, 20, 20, 40, 40]
...
for z in range(len(steps)):
    ...
    parm = ["--batch_size", "%d"%(bs)]
    args = parser.parse_args(args = parm)

    indices = np.arange(10000)

    start_time = time.time()
    for i in range(min(100, 10000//step)):
        unsolved = indices[i*step:(i+1)*step].copy()
        for j in range(3):
            unsolved, s_acc, d_acc = run(args, unsolved, sudoku, sudoku5678, ori_label, ori_label5678)
            print(s_acc, d_acc)

    print("finish-step=%d"%(step), time.time()-start_time)
