...
if (os.path.isfile(pred_path) and os.path.isfile(sep_path) and (not "new" in sys.argv)):
    print("loading from old-models")
...
if use_cuda:
    gen_labels = torch.LongTensor(np.arange(nums**3 * 2 * batch_size, dtype = "int32") % (2*nums) + base1).cuda()
else:
